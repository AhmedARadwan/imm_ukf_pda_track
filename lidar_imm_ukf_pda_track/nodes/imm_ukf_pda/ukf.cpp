  ...
  for (int motion_ind = 0; motion_ind < num_motion_model_; motion_ind++)
  {
    ...
    for (size_t i = 0; i < num_meas; i++)
    {
      ...
      if (is_direction_available){
        tf::Quaternion quat;
        quat[0] = object_vec[i].pose.orientation.x;
        quat[1] = object_vec[i].pose.orientation.y;
        quat[2] = object_vec[i].pose.orientation.z;
        quat[3] = object_vec[i].pose.orientation.w;
...
void UKF::updateMeasurementForCTRV(const std::vector<jsk_recognition_msgs::BoundingBox>& object_vec)
{
  ...
  for (auto const& object : object_vec)
  {
    ...
    if (is_direction_ctrv_available_)
    {
      ...
      tf::Quaternion quat;
      // quat = object.pose.orientation;
      quat[0] = object.pose.orientation.x;
      quat[1] = object.pose.orientation.y;
      quat[2] = object.pose.orientation.z;
      quat[3] = object.pose.orientation.w;
      double roll, pitch, yaw;
      tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);
      meas << object.pose.position.x, object.pose.position.y, yaw;
      meas_vec.push_back(meas);
...
void UKF::updateSUKF(const std::vector<jsk_recognition_msgs::BoundingBox>& object_vec)
{
...
double UKF::calculateNIS(const jsk_recognition_msgs::BoundingBox& in_object, const int motion_ind)
{
  ...
  tf::Quaternion quat;
  // quat = in_object.pose.orientation;
  quat[0] = in_object.pose.orientation.x;
  quat[1] = in_object.pose.orientation.y;
  quat[2] = in_object.pose.orientation.z;
  quat[3] = in_object.pose.orientation.w;
  double roll, pitch, yaw;
  tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);
  // Pick up yaw estimation and yaw variance
  double diff = yaw - z_pred(2);
  double nis = diff * s_pred(2, 2) * diff;

  return nis;
}

bool UKF::isLaneDirectionAvailable(const jsk_recognition_msgs::BoundingBox& in_object, const int motion_ind,
...
void UKF::checkLaneDirectionAvailability(const jsk_recognition_msgs::BoundingBox& in_object,
...
void UKF::update(const bool use_sukf, const double detection_probability, const double gate_probability,
                 const double gating_thres, const std::vector<jsk_recognition_msgs::BoundingBox>& object_vec)
{
  if (use_sukf)
  {
