...
void ImmUkfPda::run()
{
  pub_object_array_ = node_handle_.advertise<jsk_recognition_msgs::BoundingBoxArray>("/detection/objects", 1);
...
void ImmUkfPda::callback(const jsk_recognition_msgs::BoundingBoxArray& input)
{
  ...
  jsk_recognition_msgs::BoundingBoxArray transformed_input;
  jsk_recognition_msgs::BoundingBoxArray detected_objects_output;
...
void ImmUkfPda::transformPoseToGlobal(const jsk_recognition_msgs::BoundingBoxArray& input,
                                      jsk_recognition_msgs::BoundingBoxArray& transformed_input)
{
  transformed_input.header = input_header_;
  for (auto const &object: input.boxes)
  {
    geometry_msgs::Pose out_pose = getTransformedPose(object.pose, local2global_);

    jsk_recognition_msgs::BoundingBox dd;
    dd.header = input.header;
    dd = object;
    dd.pose = out_pose;

    transformed_input.boxes.push_back(dd);
  }
}

void ImmUkfPda::transformPoseToLocal(jsk_recognition_msgs::BoundingBoxArray& detected_objects_output)
{
  ...
  for (auto& object : detected_objects_output.boxes)
  {
...
void ImmUkfPda::measurementValidation(const jsk_recognition_msgs::BoundingBoxArray& input, UKF& target,
                                      const bool second_init, const Eigen::VectorXd& max_det_z,
                                      const Eigen::MatrixXd& max_det_s,
                                      std::vector<jsk_recognition_msgs::BoundingBox>& object_vec,
                                      std::vector<bool>& matching_vec)
{
  ...
  {
    double x = input.boxes[i].pose.position.x;
    double y = input.boxes[i].pose.position.y;
  ...
  if (exists_smallest_nis_object)
  {
    ...
    if (use_vectormap_ && has_subscribed_vectormap_)
    {
      jsk_recognition_msgs::BoundingBox direction_updated_object;
...
bool ImmUkfPda::updateDirection(const double smallest_nis, const jsk_recognition_msgs::BoundingBox& in_object,
                                    jsk_recognition_msgs::BoundingBox& out_object, UKF& target)
{
...
bool ImmUkfPda::storeObjectWithNearestLaneDirection(const jsk_recognition_msgs::BoundingBox& in_object,
                                                 jsk_recognition_msgs::BoundingBox& out_object)
{
  ...
  out_object.pose.orientation.x = quat.getX();
  out_object.pose.orientation.y = quat.getY();
  out_object.pose.orientation.z = quat.getZ();
  out_object.pose.orientation.w = quat.getW();
  return success;
}

void ImmUkfPda::updateTargetWithAssociatedObject(const std::vector<jsk_recognition_msgs::BoundingBox>& object_vec,
                                                 UKF& target)
{
  target.lifetime_++;
  // if (target.object_.label != 0)
  // {
...
void ImmUkfPda::updateBehaviorState(const UKF& target, const bool use_sukf, jsk_recognition_msgs::BoundingBox& object)
{
  // if(use_sukf)
  // {
...
void ImmUkfPda::initTracker(const jsk_recognition_msgs::BoundingBoxArray& input, double timestamp)
{
  for (size_t i = 0; i < input.boxes.size(); i++)
  {
    double px = input.boxes[i].pose.position.x;
    double py = input.boxes[i].pose.position.y;
...
void ImmUkfPda::secondInit(UKF& target, const std::vector<jsk_recognition_msgs::BoundingBox>& object_vec, double dt)
{
...
void ImmUkfPda::updateTrackingNum(const std::vector<jsk_recognition_msgs::BoundingBox>& object_vec, UKF& target)
{
  if (object_vec.size() > 0)
  {
...
bool ImmUkfPda::probabilisticDataAssociation(const jsk_recognition_msgs::BoundingBoxArray& input, const double dt,
                                             std::vector<bool>& matching_vec,
                                             std::vector<jsk_recognition_msgs::BoundingBox>& object_vec, UKF& target)
{
  double det_s = 0;
...
void ImmUkfPda::makeNewTargets(const double timestamp, const jsk_recognition_msgs::BoundingBoxArray& input,
...
ImmUkfPda::removeRedundantObjects(const jsk_recognition_msgs::BoundingBoxArray& in_detected_objects,
                            const std::vector<size_t> in_tracker_indices)
{
  if (in_detected_objects.boxes.size() != in_tracker_indices.size())
    return in_detected_objects;

  jsk_recognition_msgs::BoundingBoxArray resulting_objects;
  resulting_objects.header = in_detected_objects.header;

  std::vector<geometry_msgs::Point> centroids;
  //create unique points
  for(size_t i=0; i<in_detected_objects.boxes.size(); i++)
  {
    if(!isPointInPool(centroids, in_detected_objects.boxes[i].pose.position))
    {
      centroids.push_back(in_detected_objects.boxes[i].pose.position);
    }
  }
  //assign objects to the points
  std::vector<std::vector<size_t>> matching_objects(centroids.size());
  for(size_t k=0; k<in_detected_objects.boxes.size(); k++)
  {
    const auto& object=in_detected_objects.boxes[k];
  ...
  for(size_t i=0; i< matching_objects.size(); i++)
  {
    ...
    jsk_recognition_msgs::BoundingBox best_object;
    best_object = in_detected_objects.boxes[oldest_object_index];
    ...
    resulting_objects.boxes.push_back(best_object);
  }
...
void ImmUkfPda::makeOutput(const jsk_recognition_msgs::BoundingBoxArray& input,
                           const std::vector<bool> &matching_vec,
                           jsk_recognition_msgs::BoundingBoxArray& detected_objects_output)
{
  jsk_recognition_msgs::BoundingBoxArray tmp_objects;
  ...
  for (size_t i = 0; i < targets_.size(); i++)
  {
    ...
    {
      tmp_objects.boxes.push_back(dd);
...
void ImmUkfPda::dumpResultText(jsk_recognition_msgs::BoundingBoxArray& detected_objects)
{
  ...
  {
    double yaw = tf::getYaw(detected_objects.boxes[i].pose.orientation);
    ...
    outputfile << std::to_string(frame_count_) << " " << std::to_string(detected_objects.boxes[i].label) << " "
               << "Unknown"
               << " "
               << "-1"
               << " "
               ...
               << " "
               << "-1 -1 -1 -1"
               << " " << std::to_string(detected_objects.boxes[i].dimensions.x) << " "
               << std::to_string(detected_objects.boxes[i].dimensions.y) << " "
               << "-1"
               << " " << std::to_string(detected_objects.boxes[i].pose.position.x) << " "
               << std::to_string(detected_objects.boxes[i].pose.position.y) << " "
               << "-1"
               << " " << std::to_string(yaw) << "\n";
  }
  frame_count_++;
...
void ImmUkfPda::tracker(const jsk_recognition_msgs::BoundingBoxArray& input,
                        jsk_recognition_msgs::BoundingBoxArray& detected_objects_output)
{
  double timestamp = input.header.stamp.toSec();
  std::vector<bool> matching_vec(input.boxes.size(), false);
